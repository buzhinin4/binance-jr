...
export class IndicatorsService {
  ema(klines: BinanceKline[], period: number): IndicatorValue[] {
    ...
    result.push({
      time: klines[Math.min(period - 1, klines.length - 1)].openTime,
      value: ema,
    });
  ...
  sma(klines: BinanceKline[], period: number): IndicatorValue[] {
